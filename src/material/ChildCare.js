...
export const innerElements = [<circle cx={14.5} cy={10.5} r={1.25} key="k0" />
,
<circle cx={9.5} cy={10.5} r={1.25} key="k1" />
,
...
const ChildCare = ({ title, ...restProps }) => {
  return React.createElement(
    ...
    ),
    {
      children: (
        title
          ? [<title key="ChildCare-title">{title}</title>, ...innerElements]
          : innerElements
      ),
      viewBox: '0 0 24 24',
       'aria-hidden': title ? null : 'true',
       'aria-labelledby': title && 'icon-title-ChildCare',
      focusable: 'false',
...
ChildCare.defaultProps = { height: 24, width: 24}
