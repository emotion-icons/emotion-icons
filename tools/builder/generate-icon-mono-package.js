...
async function run() {
  const sources = await fg('packages/@emotion-icons/*/manifest.json', {cwd: baseDir})
  ...
  for (const pack of packs) {
    ...
    await fs.writeFile(path.join(packName, 'index.ts'), `export * from '@emotion-icons/${packName}'`)
  }
...
`,
  ...
  for (const icon of icons) {
    icon.importPath = icon.importPath.replace('@emotion-icons', 'emotion-icons')
  }
